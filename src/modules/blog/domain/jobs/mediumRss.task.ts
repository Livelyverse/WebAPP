...
export class MediumRssTask {
  ...
  fetchMediumRss() {
    this._httpService.get(this._mediumRssAddress).pipe(
      RxJS.map((response) => {
      ...
      error: (error) => this._logger.error(`fetchMediumRss fetch RSS failed\n error: ${error.stack}`),
      complete: () => this._logger.log(`fetchMediumRss fetch RSS completed`)
    });
  }
}

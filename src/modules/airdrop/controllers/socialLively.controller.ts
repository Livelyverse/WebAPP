...
@ApiBearerAuth()
@ApiTags('/api/airdrop/lively/social/profile')
@Controller('/api/airdrop/lively/social/profile')
export class SocialLivelyController {
  ...
  @ApiParam({
    name: 'param',
    required: true,
    description: `either an uuid or one of the ${Object.keys(SocialType)}`,
    schema: { oneOf: [{ enum: Object.keys(SocialType) }, { type: 'uuid' }] },
  })
  ...
  findSocialLively(@Param() params): RxJS.Observable<SocialLivelyViewDto> {
    return RxJS.merge(
      ...
      RxJS.of(params).pipe(
        ...
        RxJS.mergeMap(pathParam => this._socialLivelyService.findBySocialType(pathParam.param)),
        RxJS.map(entity => SocialLivelyViewDto.from(entity)),
      ),
      RxJS.of(params).pipe(
